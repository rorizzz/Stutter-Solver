...
class SynthesizerTrn(nn.Module):
    ...
    def infer_prolong(self, x, x_lengths, bert, sid=None, noise_scale=1, length_scale=1, max_len=None):
        len = x.size(1)
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, bert)
        if self.n_speakers > 0:
            g = self.emb_g(sid).unsqueeze(-1)  # [b, h, 1]
        ...
        # prolong
        # index_in_s, index = get_multiple_vowel(s)
        index = random.randrange(3, len-3, 2)
        # print("index: ", index)
        if index != None:
            # print(index) 
            prolong_fator = random.randint(5, 8)
        ...
        z = self.flow(z_p, y_mask, g=g, reverse=True)
        o = self.dec((z * y_mask)[:, :, :max_len], g=g)
        return o, attn, y_mask, (z, z_p, m_p, logs_p), index, w_ceil
